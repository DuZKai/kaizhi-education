...
from util import use_urls_get_img
...
def get_teacher_sql(teacher, teacher_name, teacher_index, company_random_code, result_folder):
...
def get_course_teacher_sql(course_teacher_index, index, teacher_index):
    return "INSERT INTO `course_teacher` VALUES (" + str(
        course_teacher_index) + ", '" + str(index) + "', '" + str(teacher_index) + "');"


def get_chapter_and_teachplan(index, chapter_index, teachplan_media_index, chapter_sqls, teachplan_media_sqls, add_minio_files, body_soup, minio_id_name_map):
    big_order_by = 1
    small_order_by = 1
    big_num = 0
    ...
    for chapter in directory.split('=*=')[0: -1]:
        ...
        for small_chapter in all_chapter[1:]:
            ...
            if add_minio_files:
                ...
                id = id_name[0]
                name = id_name[-1]
                teachplan_media_sql = "INSERT INTO `teachplan_media` VALUES (" + str(
                    teachplan_media_index) + ", '" + id + "', " + str(chapter_index) + ", " + str(
                    index) + ", '" + name + "', '2024-12-03 17:22:19', NULL, NULL);"
        ...
        if big_num > 2:
            break
    return chapter_index, teachplan_media_index
...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        if (html.startswith('http')):
            print("正在处理第" + str(index) + "个课程：" + html)
            body_soup = get_soup(html)
            # company_random_code = random.choice(['1232141425', '1'])
            company_random_code = '1232141425'
            base_sql = get_base_sql(index, company_random_code, body_soup, original)
            base_sqls.append(base_sql)

            market_sql = get_market_sql(index)
            ...
            for teacher_info in teachers_info.split('=*='):
                ...
                if teacher_name in teacher_map:
                    course_teacher_sql = get_course_teacher_sql(course_teacher_index, index, teacher_map[teacher_name])
                    course_teacher_sqls.append(course_teacher_sql)
                    course_teacher_index += 1
                else:
                    teacher_map[teacher_name] = teacher_index
                    teacher_sql = get_teacher_sql(teacher, teacher_name, teacher_index, company_random_code, result_folder)
                    teacher_sqls.append(teacher_sql)

                    course_teacher_sql = get_course_teacher_sql(course_teacher_index, index, teacher_index)
                    course_teacher_sqls.append(course_teacher_sql)
                    teacher_index += 1
                    course_teacher_index += 1

            chapter_index, teachplan_media_index = get_chapter_and_teachplan(
                index, chapter_index, teachplan_media_index, chapter_sqls, teachplan_media_sqls,
                add_minio_files, body_soup, minio_id_name_map)
        index += 1
